-- Historical mRemoteNG SQL Server schema, extracted verbatim from commit b9f2abf5e
-- ("add db schema initialization on a new empty database", BlueBlock, 2023-03-01).
--
-- This is NOT generated by the current build. That distinction is the whole point: a schema this
-- build produces cannot carry the column ordering, types, nullability or auto-named constraints of
-- a database a user actually has, so replaying migrations against one proves only that the code
-- agrees with itself. Every migration defect this project shipped (#113, #146, #147, #148, #165)
-- was found by a reporter running a real, older database.
--
-- Applied by SqlSchemaMigrationLiveTests against a throwaway database, then upgraded to current.
