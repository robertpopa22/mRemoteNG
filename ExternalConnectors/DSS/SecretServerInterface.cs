...
public static class SecretServerInterface
{
    ...
    private sealed class PasswordFinder(string password) : IPasswordFinder
    {
        ...
        public static RSACryptoServiceProvider ImportPrivateKey(string pem)
        {
            ...
            // The key size comes from the key being imported, not from a default. This method
            // creates no key material: it converts an existing private key, supplied by the user's
            // credential vault, into the format PuTTY expects. Deriving the size from the modulus
            // states that explicitly instead of leaving a parameterless constructor that reads like
            // weak key generation (S4426).
