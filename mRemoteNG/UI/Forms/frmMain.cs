...
namespace mRemoteNG.UI.Forms
{
    ...
    public partial class FrmMain : IMessageFilter
    {
                ...
                switch (m.Msg)
                {
                    ...
                    case NativeMethods.WM_ACTIVATE:
                        ...
                        if (NativeMethods.LOWORD(m.WParam) == NativeMethods.WA_CLICKACTIVE)
                        {
                            ...
                            {
                                bool focusedNonConnection = false;
                                ...
                                {
                                    // Don't SimulateClick or Focus() — PropertyGridView starts cell
                                    // edit mode on the real WM_LBUTTONDOWN/UP sequence and transfers
                                    // keyboard focus to its in-place editor. A synthetic click or a
                                    // Focus() on PropertyGridView itself steals focus back from the
                                    // editor and prevents the user from typing into the cell.
                                    focusedNonConnection = true;
                                }
                                         ...
                                         controlThatWasClicked is ToolStrip)
                                {
                                    ...
                                    // ConnectionTree (TreeListView : ListView) needs this so the
                                    // click reaches the listview — otherwise +/- glyph expand and
                                    // cell-edit clicks are lost while an RDP connection is open.
                                    // ObjectListView blank-selection from RDP focus battle is still
                                    // corrected by _pendingClickTarget in ConnectionTree.WndProc (#68).
                                ...
                                // Cancel the pending reactivation restore when the click already focused a
                                // non-connection control. Otherwise the next WM_WINDOWPOSCHANGED would
                                // re-run ActivateConnection() and yank focus back to the protocol — making
                                // the tree / PropertyGrid unusable while a connection is open.
                                if (focusedNonConnection)
                                    _pendingActivateConnectionOnAppReactivation = false;
                            }
                        }
                        break;
        ...
        private void QueueActivateConnection()
        {
            ...
            try
            {
                BeginInvoke((MethodInvoker)TryRestorePendingConnectionFocus);
            }
        ...
        // Guarded reactivation restore. Called both from the BeginInvoke posted by
        // QueueActivateConnection (Alt+Tab path) and from WM_WINDOWPOSCHANGED. Only
        // refocuses the active connection if the pending flag is still set — so that
        // a click on a non-connection control (WA_CLICKACTIVE branch) can cancel the
        ...
        private void TryRestorePendingConnectionFocus()
        {
            ...
            // messages don't retry the restore behind the user's back.
            if (IsCursorOverConfigWindow())
            {
                _pendingActivateConnectionOnAppReactivation = false;
                return;
            }
        ...
        private static bool IsInsidePropertyGrid(Control? c)
        {
            for (Control? cur = c; cur != null; cur = cur.Parent)
                if (cur is PropertyGrid) return true;
            return false;
        }

        internal static bool IsCursorOverConfigWindow()
        {
            try
            {
                Control? ctrl = FromChildHandle(NativeMethods.WindowFromPoint(MousePosition));
                for (Control? c = ctrl; c != null; c = c.Parent)
                    if (c is ConfigWindow) return true;
            }
            catch (Exception ex)
            {
                _ = ex; // ignore — best-effort guard
            }
            return false;
        }

        internal bool IsCursorOverMainWindowNonClientArea()
        {
            try
            {
        ...
        {
            // Never steal focus from the PropertyGrid while the user is editing
            // a connection property. Multiple paths call ActivateConnection()
            // (reactivation, size/move end, tab changes) — guard all of them.
            if (IsCursorOverConfigWindow()) return;
        ...
        internal void UpdateWindowTitle()
        {
                ...
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(asm))
                        titleBuilder.Append(CultureInfo.CurrentCulture,
                                            $" [DEV {System.IO.File.GetLastWriteTime(asm):HH:mm}]");
                    else
            ...
            if (!string.IsNullOrEmpty(SelectedConnection?.Name))
            {
                ...
                // Also respect an explicit user selection: when the tree already has a
                // different node selected, don't clobber it when focus is in the
                // PropertyGrid editing that node's fields.
                if (Settings.TrackActiveConnectionInConnectionTree)
                {
                    var treeForm = AppWindows.TreeForm;
                    var tree = treeForm?.ConnectionTree;
                    if (tree?.Focused != true)
                    {
                        bool userPickedOtherNode =
                            tree?.SelectedObject is ConnectionInfo sel &&
                            sel != SelectedConnection;
                        if (!userPickedOtherNode)
                            treeForm?.JumpToNode(SelectedConnection, suppressPreview: true);
                    }
                }
            }
